...
# kvasir.storm.encoder opens a litellm disk cache under Path.home() while being imported, so a
# test run would otherwise litter the developer's home with ~/.storm_local_cache. This has to
# happen before any test module imports kvasir.storm, which conftest collection guarantees. The
