...
class FindRelatedTopic(dspy.Signature):
    ...
    related_topics: list[str] = dspy.OutputField(
        desc="URLs of Wikipedia pages on closely related subjects"
    )
...
class GenPersona(dspy.Signature):
    ...
    examples: str = dspy.InputField(
        desc="wiki page outlines of related topics, for inspiration"
    )
    personas: list[str] = dspy.OutputField(
        desc="one editor per item, written as 'short summary of the editor: description'"
    )
...
class CreateWriterWithPersona(dspy.Module):
    ...
    def forward(self, topic: str, draft=None):
        with dspy.settings.context(lm=self.engine):
            ...
            examples = []
            for url in [line for line in related_topics if line.startswith("http")]:
                try:
        ...
        return dspy.Prediction(personas=personas, related_topics=related_topics)
...
class StormPersonaGenerator:
    ...
    Args:
        engine (dspy.LM): The underlying engine used for generating
            personas. It must be an instance of either `dspy.LM`.
    """
